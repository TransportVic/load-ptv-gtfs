import GTFSShapePoint from '../GTFSShapePoint.mjs'
...
export default class GTFSShapeReader {
  ...
  async getNextEntity() {
    ...
    let shapeLength = points[points.length - 1].distance

    return {
      shapeID,
      points,
      shapeLength,
      hash: `${shapeLength}-${points[0].coordinates.join(',')}-${points[points.length - 1].coordinates.join(',')}`
    }
  }
   ...
   * 
   * @returns {GTFSShapePoint} An object representing the shape data
   */
  processEntity(data) {
    let shapeData = {
    ...
    return new GTFSShapePoint(shapeData)
  }
}