...
export default class GTFSCalendarReader extends GTFSReader {
  ...
  #calendarDateFile
  #calendarDates = {}
  #unprocessedCalendarDates = new Set()
  ...
  #checkMissingFile(e) {
    if (!e.stack.includes('ENOENT')) throw e
  }
  ...
  async open() {
    ...
    try {
      ...
      while (calendarDateReader.available()) {
        ...
        if (!this.#calendarDates[calendarDate.id]) {
          this.#calendarDates[calendarDate.id] = []
          this.#unprocessedCalendarDates.add(calendarDate.id)
        }
        this.#calendarDates[calendarDate.id].push(calendarDate)
      }
    } catch (e) { this.#checkMissingFile(e) }
  }
  ...
  processEntity(data) {
    ...
    this.#unprocessedCalendarDates.delete(data.service_id)

    return new GTFSCalendar(calendarData, this.#calendarDates[data.service_id])
  }

  available() {
    const underlyingAvailable = this.#hasCalendar ? super.available() : false
    if (underlyingAvailable) return true
    return this.#unprocessedCalendarDates.size > 0
  }

  getNextEntity() {
    if (this.#hasCalendar && super.available()) return super.getNextEntity()
    const calendarDate = Array.from(this.#unprocessedCalendarDates.values())[0]
    const calendarData = this.#calendarDates[calendarDate]

    const calendar = new GTFSCalendar({
      id: calendarDate,
      daysOfWeek: Array(7).fill(false),
      startDay: calendarData[0].rawDate,
      endDay: calendarData[0].rawDate
    }, calendarData)

    this.#unprocessedCalendarDates.delete(calendarDate)
    return calendar
  }
}