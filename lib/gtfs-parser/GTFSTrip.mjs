...
export default class GTFSTrip {
  ...
  static create(data, mode) {
    let types = [ SmartrakTrip, MetroTrip, MTMRailTrip, GTFSTrip ]
...
export class MetroTrip extends GTFSTrip {
  ...
  constructor(data) {
    ...
    let runID = data.id.match(/^02-\w{3}-.*-(\w{1,2}\d{2,3})$/)
    this.#runID = runID[1]
  }

  static canProcess(data, mode) {
    return mode === TRANSIT_MODES.metroTrain && data.id.match(/^02-\w{3}-.*-(\w{1,2}\d{2,3})$/)
  }
...
export class MetroRRBTrip extends MetroTrip {
  ...
  static canProcess(data, mode) {
    return super.canProcess(data, mode) && data.id.match(/^02-\w{3}-R-/)
  }

}

export class MTMRailTrip extends GTFSTrip {
