...
export default class GTFSCalendar {
  ...
  constructor({ id, daysOfWeek, startDay, endDay }, calendarDates = []) {
    ...
    this.startDay = dateUtils.parseDate(startDay)
    this.endDay = dateUtils.parseDate(endDay)
  ...
  /**
   * Gets the operation days, taking into account any calendar date exceptions
   * @returns {string[]} A string array of GTFS formatted dates
   */
  getOperationDays() {
    return this.#operationDays.sort((a, b) => a - b).map(day => dateUtils.toGTFSDate(day)).filter((date, i, arr) => i === 0 || arr[i - 1] != date)
  }
