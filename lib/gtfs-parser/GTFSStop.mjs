...
export class GTFSStop {
  ...
  constructor({ stopGTFSID, stopName, stopLat, stopLon, locationType, parentStopGTFSID }, suburbs, suburbHook) {
  ...
  #setSuburb(suburbs, suburbHook) {
    let output
    if (suburbHook && (output = suburbHook(this))) {
      this.suburb = output
    } else if (this.requiresSuburb()) {
  ...
  static create(data, suburbs, suburbHook) {
    let types = [ GTFSRRBStop, GTFSPlatformStop, GTFSStop ]
    for (let type of types) {
      if (type.canProcess(data)) return new type(data, suburbs, suburbHook)
    }
  }
}
...
export class GTFSPlatformStop extends GTFSStop {
  ...
  constructor(data, suburbs, suburbHook) {
