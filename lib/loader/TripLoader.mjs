...
export default class TripLoader {
  ...
  constructor({ tripsFile, stopTimesFile, calendarFile, calendarDatesFile }, mode, database) {
    ...
    this.#calendarDatesFile = calendarDatesFile
  }
  ...
  async generateDBTripData(tripData, stops) {
    ...
    let stopTimings = await async.map(stops, async stop => {
      ...
      if (!bay) throw new Error(`Missing bay for given stop ID ${stop.stopID} in trip ${tripData.getTripID()}`)
  ...
  async loadTrips({ routeIDMap, processTrip, onWrite = () => {} }) {
    ...
    this.#stopTimesReader = this.createStopTimesReader(this.#stopTimesFile, this.#mode)
    this.#tripsReader = this.createTripReader(this.#tripsFile, null, null, this.#mode)
    this.#calendarReader = this.createCalendarReader(this.#calendarFile, this.#calendarDatesFile)
