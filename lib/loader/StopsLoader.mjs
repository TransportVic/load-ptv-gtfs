...
export default class StopsLoader {
   ...
   * @param {string} collectionName The collection to store the stops in. Defaults to gtfs-stops
   * 
   */
  constructor(stopsFile, suburbs, mode, database, suburbHook, collectionName = 'gtfs-stops') {
    this.#stops = database.getCollection(collectionName)
    this.#mode = mode
    this.#stopsReader = new GTFSStopsReader(stopsFile, suburbs, suburbHook)
  }
