...
export default async function setRouteStops(database, directionsMap, onMilestone = () => {}, {
  ...
  let totalRouteCount = await routes.countDocuments({})
  let total = totalRouteCount
  let count = 0

  let stopsByService = {}

  for (let mode of Object.values(TRANSIT_MODES)) {
    let allRoutes = await routes.distinct('routeGTFSID', { mode })

    await async.forEachLimit(allRoutes, 30, async routeGTFSID => {
      let routeData = await routes.findDocument({ routeGTFSID }, { 'routePath.path': 0 })
      ...
      for (let variant of routeVariants) {
        let timetable = await gtfsTimetables.findDocument(variant, { gtfsDirection: 1, stopTimings: 1 })
        if (!timetable) {
